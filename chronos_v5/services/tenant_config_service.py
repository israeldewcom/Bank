...
class TenantConfigService:
    ...
    def get_config(self, tenant: str) -> dict:
        ...
        try:
            ...
            db.rollback()
            db.close()
            logger.warning(f"tenant_configs table missing: {e}")
            defaults = self._get_defaults(tenant)
            self.redis.setex(cache_key, self.cache_ttl, json.dumps(defaults))
            return defaults
        except Exception as e:
            db.rollback()
            db.close()
            logger.error(f"TenantConfig query failed: {e}")
            defaults = self._get_defaults(tenant)
            return defaults
    ...
    def _get_defaults(self, tenant: str) -> dict:
        return {
            "tenant": tenant,
            "performance_fee_percent": Config.PERFORMANCE_FEE_PERCENT,
    ...
    def update_config(self, tenant: str, updates: dict):
        ...
        try:
            ...
            db.rollback()
            logger.error(f"Update tenant config failed: {e}")
            raise
        finally:
            db.close()
