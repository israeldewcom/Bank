...
class MarketDataAggregator:
    ...
    def get_price(self, symbol: str) -> float:
        ...
        self.redis.setex(f"price:{symbol}", 60, price)
        return price
    ...
    def _fetch_from_provider(self, symbol: str):
        if Config.MARKET_DATA_PROVIDER == "bloomberg" and Config.BLOOMBERG_API_KEY:
            try:
                resp = requests.get(f"{Config.BLOOMBERG_API_URL}/securities/{symbol}/price",
                                    headers={"Authorization": Config.BLOOMBERG_API_KEY},
                                    timeout=Config.MARKET_DATA_TIMEOUT)
                resp.raise_for_status()
                return float(resp.json()['price'])
            except Exception as e:
                logger.warning(f"Bloomberg fetch failed for {symbol}: {e}")
        elif Config.MARKET_DATA_PROVIDER == "reuters" and Config.REUTERS_API_KEY:
            try:
                resp = requests.get(f"{Config.REUTERS_API_URL}/prices/{symbol}",
                                    headers={"Authorization": f"Bearer {Config.REUTERS_API_KEY}"},
                                    timeout=Config.MARKET_DATA_TIMEOUT)
                resp.raise_for_status()
                return float(resp.json()['price'])
            except Exception as e:
        ...
        if Config.ALPHA_VANTAGE_API_KEY:
            try:
                resp = requests.get(f"https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol={symbol}&apikey={Config.ALPHA_VANTAGE_API_KEY}",
                                    timeout=Config.MARKET_DATA_TIMEOUT)
                resp.raise_for_status()
                data = resp.json()
    ...
    def compute_alpha(self) -> dict:
        ...
        if Config.ALPHA_STRATEGY_TYPE == "mean_reversion":
            ...
            for asset in Config.ALPHA_STRATEGY_ASSETS:
                price = self.get_price(asset)
                old_price = self.redis.get(f"price_old:{asset}")
                if old_price:
                    change = (price - float(old_price)) / float(old_price)
                    signals[asset] = change
                else:
                    signals[asset] = 0.0
                self.redis.setex(f"price_old:{asset}", 600, price)
        for asset, value in signals.items():
            self.redis.setex(f"alpha:{asset}", 60, value)
        return signals
